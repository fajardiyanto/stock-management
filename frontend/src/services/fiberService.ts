...
import { apiCall } from "./";
import { FiberRequest, FiberResponse, FiberPaginationResponse, FiberFilter } from "../types/fiber";

export const fiberService = {
    getAllFiber: async (filters: FiberFilter = {}): Promise<ApiResponse<FiberPaginationResponse>> => {
    ...
    createFiber: async (data: FiberRequest): Promise<ApiResponse<FiberResponse>> => {
        const response = await apiCall<ApiResponse<FiberResponse>>(
            `/fiber`,
            {
                method: 'POST',
    ...
    updateFiber: async (id: string, data: FiberRequest): Promise<ApiResponse<any>> => {
        const response = await apiCall<ApiResponse<any>>(
            `/fiber/${id}`,
            {
                method: 'PUT',
