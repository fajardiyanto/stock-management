...
interface FiberModalDeleteProps {
    name: string;
    onConfirm: () => void;
    onClose: () => void;
}

const FiberModalDelete: React.FC<FiberModalDeleteProps> = ({ name, onConfirm, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md transform transition-all duration-300 scale-100">
                ...
                <p className="text-gray-700 mb-6 border-t pt-4">
                    Are you absolutely sure you want to delete fiber <b>{name}</b>?
                </p>
...
export default FiberModalDelete;