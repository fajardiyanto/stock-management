...
interface ManualEntryFormProps {
    ...
    onChange: (id: string, field: keyof ManualEntryFormRequest, value: string | number) => void;
    onRemove: (id: string) => void;
}

const ManualEntryForm: React.FC<ManualEntryFormProps> = ({ index, entry, onChange, onRemove }) => {
    const [formattedAmount, setFormattedAmount] = useState(() => formatRupiah(entry.total));
    ...
    return (
        <div className="border border-gray-300 rounded-xl p-4 shadow-sm">
            <h5 className="text-md font-semibold text-gray-700 mb-3 border-b border-gray-100 pb-2">Kelola Keuangan #{index + 1}</h5>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    ...
                    <div className="relative">
                        ...
                        <X size={16} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 cursor-pointer"
                            onClick={() => { onChange(entry.tempId, 'total', 0); setFormattedAmount(''); }} />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        <select
                            value={entry.type}
                            onChange={(e) => onChange(entry.tempId, 'type', e.target.value as 'INCOME' | 'EXPENSE')}
                ...
                <div className="md:col-span-2 flex items-end space-x-2">
                    <div className="flex-grow">
                        ...
                        <input
                            ...
                            onChange={(e) => onChange(entry.tempId, 'description', e.target.value)}
