...
const StockRowDetails: React.FC<StockRowDetailsProps> = ({
    ...
    const calculateTotalRowsForStock = () => {
        ...
        stockEntry.stock_items.forEach(stockItem => {
            if (stockItem.is_sorted && stockItem.stock_sorts) {
                const marketable = stockItem.stock_sorts.filter(s => !s.is_shrinkage);
                const hasShrinkage = stockItem.stock_sorts.some(s => s.is_shrinkage);
                total += Math.max(1, marketable.length) + (hasShrinkage ? 1 : 0);
            } else {
                total += 1;
            }
        });
    ...
    const renderStockEntryColumns = (rowSpan: number) => (
        <React.Fragment>
            <td rowSpan={rowSpan} className="px-6 py-4 text-sm font-bold text-gray-900 align-middle text-center border-r border-gray-200">
    ...
    const renderItemDetailsColumns = (rowSpan: number | null) => (
        <React.Fragment>
            <td rowSpan={rowSpan || 1} className="px-4 py-4 text-sm text-gray-800 font-semibold text-center align-middle border-l border-gray-200">
                {item.item_name}
            </td>
            <td rowSpan={rowSpan || 1} className="px-2 py-4 text-sm text-gray-700 text-center align-middle">
                {item.weight}
            </td>
            <td rowSpan={rowSpan || 1} className="px-2 py-4 text-sm text-gray-700 text-center align-middle">
                {formatRupiah(item.price_per_kilogram)}
            </td>
            <td rowSpan={rowSpan || 1} className="px-2 py-4 text-sm font-semibold text-gray-900 text-center align-middle border-r border-gray-200">
                {formatRupiah(item.total_payment)}
            </td>
        </React.Fragment>
    );

    const renderSortActionColumn = (actionText: string, rowSpan: number | null = null) => (
        <td rowSpan={rowSpan || 1} className="px-4 py-4 text-center border-l border-gray-200 align-middle">
            <button
                onClick={onSort}
    ...
    const renderStockActionsColumn = (rowSpan: number) => (
        <td rowSpan={rowSpan} className="px-6 py-4 align-middle border-l border-gray-200">
    ...
    if (!isSorted) {
        return (
            <tr className={`bg-white hover:bg-gray-50 transition ${rowClass}`}>
                ...
                <td className="px-2 py-4 text-sm text-gray-700 text-center border-l border-gray-200">-</td>
                <td className="px-2 py-4 text-sm text-gray-700 text-center">-</td>
                <td className="px-2 py-4 text-sm font-semibold text-gray-700 text-center">-</td>
                <td className="px-2 py-4 text-sm font-bold text-green-600 text-center">-</td>
                <td className="px-2 py-4 text-sm font-semibold text-gray-900 text-center border-r border-gray-200">-</td>
        ...
        <React.Fragment>
            {marketableSorts.map((sortItem, index) => {
                ...
                return (
                    ...
                    >
                        {isFirstRow && isFirstItemRow && renderStockEntryColumns(totalStockRows)}

                        {isFirstItemRow ? renderItemDetailsColumns(totalItemRows) : null}
                        ...
                        {isFirstItemRow && renderSortActionColumn('Edit Sortir', totalItemRows)}

                        {isFirstRow && isFirstItemRow && renderStockActionsColumn(totalStockRows)}
                    </tr>
                );
            })}
