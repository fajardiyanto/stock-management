...
const StockSortResultInput: React.FC<StockSortResultInputProps> = ({
    index,
    result,
    onChange,
    onRemove,
    ...
    const [formattedPrice, setFormattedPrice] = useState<string>(
        formatRupiah(result.price_per_kilogram)
    );
    ...
    const handleValueChange = (
        ...
        const parsedValue =
            field === "sorted_item_name" ? value : parseFloat(value) || 0;
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <div className="md:col-span-2 flex items-center justify-between h-[42px] mt-4 md:mt-0">
                    ...
                    <button
                        ...
                        className={`w-16 h-8 rounded-full p-0.5 transition-colors duration-200 flex-shrink-0 ${
                            result.is_shrinkage
                    ...
                    >
                        <span
                            className={`w-7 h-7 bg-white rounded-full shadow-md transform transition-transform duration-200 flex items-center justify-center`}
                        >
                            {result.is_shrinkage && (
                    ...
                    <button
                        type="button"
                        className="ml-4 px-3 py-1 bg-gray-200 text-gray-700 text-sm rounded-lg hover:bg-gray-300"
                    >
                        {!result.is_shrinkage
                            ? "Simpan Dijual"
                            : "Tidak Dijual"}
