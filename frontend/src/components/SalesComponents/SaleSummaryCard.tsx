...
const SummaryRow: React.FC<{
    ...
    <div
        className={`flex justify-between py-2 ${
            isTotal ? "border-t-2 border-gray-300 pt-3" : ""
        }`}
    >
        <span
            className={`text-sm ${
                isTotal ? "font-bold text-lg" : "text-gray-700"
            }`}
        >
            {label}
        </span>
        <span
            className={`text-sm ${
                isTotal ? "font-bold text-lg text-gray-900" : "text-gray-800"
            }`}
        >
            {value}
        </span>
...
const SaleSummaryCard: React.FC<SaleSummaryCardProps> = ({
    totalItem,
    totalAddon,
    totalFiber,
    total,
