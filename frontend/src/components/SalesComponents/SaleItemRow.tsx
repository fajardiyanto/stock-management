...
const SaleItemRow: React.FC<SaleItemRowProps> = ({ sale, item, itemIndex, totalItems, onDelete, onRefresh }) => {
    ...
    const getPaymentBadge = (status: SaleEntry['payment_status']) => {
        ...
        else if (status === 'PARTIAL_PAYMENT') style = 'bg-yellow-100 text-yellow-800';
        return <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${style}`}>{PaymentStatusLabel[status]}</span>;
    };

    return (
        <tr className={`hover:bg-gray-50 transition border-t-2 ${isFirstRow ? 'border-gray-300' : 'border-none'}`}>
            {isFirstRow && (
                <>
            ...
            <td className="px-6 py-4 font-bold text-sm text-gray-600">{item.stock_code}</td>
            <td className="px-6 py-4 text-sm text-gray-800">{item.stock_sort_name}</td>
            <td className="px-6 py-4 text-sm text-gray-600">{formatRupiah(item.price_per_kilogram)}</td>
                    ...
                    <td rowSpan={totalItems} className="px-6 py-4 text-sm align-middle">
                        ...
                        <div className="mt-1 h-1 bg-gray-200 rounded-full overflow-hidden">
                            <div
                                ...
                                style={{ width: `${(sale.paid_amount / sale.total_amount) * 100}%` }}
                            ></div>
                        </div>
