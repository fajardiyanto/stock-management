...
const UserModalDetail: React.FC<UserModalDetailProps> = ({ user, cashFlows, onClose, onEdit, onRefresh }) => {
    ...
    const [cashFlow, setCashFlow] = useState<PaymentResponse>({} as PaymentResponse);
    ...
    const handleAddForm = () => {
        ...
        setManualForms(prev => [...prev, { ...initialManualEntry, tempId: 'new-' + Math.random().toString(16).slice(2) }]);
        setManualError('');
    };
    ...
    const handleFormChange = (
        ...
        setManualForms(prev => prev.map(form => (
    ...
    const handleManualSubmit = async (e: React.FormEvent) => {
        ...
        const validEntries = manualForms.filter(f => f.total > 0 && f.description.trim() !== '');

        if (validEntries.length === 0) {
            setManualError("Harap masukkan minimal satu entri Kas yang valid (Jumlah > 0 dan Deskripsi).");
            return;
        }
        ...
        try {
            const response = await paymentService.createManualPayment(user.uuid, validEntries);
            if (response.status_code === 200) {
                await onRefresh();
            } else {
                setManualError(response.message || 'Failed to create manual payment');
                showToast(response.message || 'Failed to create manual payment', 'error');
            }
        } catch (err) {
            setManualError('Failed to create manual payment. Please try again.');
            showToast('Failed to create manual payment. Please try again.', 'error');
        } finally {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const response = await paymentService.deleteManualPayment(cashFlow.uuid);

            if (response.status_code === 200) {
                await onRefresh();
            } else {
                setManualError(response.message || 'Failed to remove manual payment');
                showToast(response.message || 'Failed to remove manual payment', 'error');
            }
        } catch (err) {
            setManualError('Failed to remove manual payment. Please try again.');
            showToast('Failed to remove manual payment. Please try again.', 'error');
        } finally {
            setModalType(null);
        }
    };
            ...
            <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-6xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100">
                    ...
                    <div className="space-y-8 pt-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 border p-4 rounded-lg bg-gray-50">
                            ...
                            <div className="sm:col-span-2">
                                <DetailRow label="Registered At" value={formatDate(user.created_at)} />
                            </div>
                        </div>
                    ...
                    <div className="mt-8 border-t pt-8">
                        <CashFlowHistoryTable user={user} cashFlows={cashFlows} onRemove={handleRemovePayment} />
                    </div>

                    <form onSubmit={handleManualSubmit} className="mt-8 border-t pt-8 space-y-4">
                        <div className="flex justify-between items-center">
                            <h4 className="text-xl font-bold text-gray-700">Kelola Keuangan</h4>
                            <button
                                type="button"
                        ...
                        {manualForms.length > 0 && (
                            <div className="flex justify-end pt-4">
                                <button
                                    ...
                                    disabled={isSubmittingManual}
                                >
                                    {isSubmittingManual ? 'Menyimpan...' : `Simpan ${manualForms.length} Entri`}
                                </button>
                            </div>
                        )}
                    </form>
                ...
                {modalType === 'DELETE' && (
                    <PaymentModalDelete item={cashFlow} onConfirm={handleConfirmDelete} onClose={handleCloseModal} />
                )}
            </div>
        </>
    );
