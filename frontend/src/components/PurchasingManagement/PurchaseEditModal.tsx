...
const PurchaseEditModal: React.FC<PurchaseEditModalProps> = ({ purchase, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await purchaseService.updatePurchase(purchase.purchase_id, finalPayload)

            if (response.status_code === 201 || response.status_code === 200) {
                showToast("Berhasil melakukan update pembelian!", "success");
            } else {
                const errorMessage = response.message || "Gagal update pembelian. Coba lagi.";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100">
                <div className="flex items-center justify-between border-b border-gray-100 p-5">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900">Edit Pembelian</h3>
                        <p className="text-sm text-gray-500">Perbarui informasi pembelian <b>${purchase.stock_code}</b></p>
                    </div>
                    <button
                        onClick={onClose}
                        type="button"
                    ...
                    <div className="space-y-8 pt-6">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                >
                                    <option key={purchase.supplier.uuid || 'all'} value={purchase.supplier.uuid}>
                        ...
                        <div>
                            <div className="flex justify-end gap-3 border-t border-gray-100 p-5 pt-4">
                                ...
                                <button
                                    ...
                                    disabled={isSubmitting}
                                >
                                    {isSubmitting ? 'Updating...' : 'Update Pembelian'}
                                </button>
                            </div>
                        </div>
                    </div>
